...
run_build() {
    ...
    lunch lineage_cmi-bp4a-userdebug
...
if [[ "$existing_graph" -eq 1 ]]; then
    ninja_file="$lineage_root/out/combined-lineage_cmi.ninja"
...
if [[ "$build_status" -eq 0 ]]; then
    case "$target" in
        bacon|otapackage|lineage_cmi-ota.zip|*/lineage_cmi-ota.zip)
            "$script_dir/verify-build-artifacts.sh" "$lineage_root" \
                "$lineage_root/out/target/product/cmi/lineage_cmi-ota.zip"
            ;;
    esac
fi
