...
usage() {
    echo "Usage: $(basename "$0") /path/to/CaCamOS-dipper-uvc-boot.img" >&2
}
...
if [[ ! -f "$apk" ]]; then
    ...
    curl -L --fail -o "$apk" "$magisk_url"
fi

patch_dir="$work_dir/dipper-patch-files"
rm -rf "$patch_dir"
mkdir -p "$patch_dir"
unzip -q -j "$apk" \
...
if [[ "$boot_base" =~ (20[0-9]{6}) ]]; then
    boot_date="${BASH_REMATCH[1]}"
    if [[ "$lineage" != *"$boot_date"* ]]; then
        cat >&2 <<EOF
Refusing to patch $boot_base.
The phone is running: ${lineage:-unknown}
The boot image appears to be for build date: $boot_date
EOF
        exit 1
    fi
else
    echo "WARNING: no build date found in $boot_base; cannot compare with ro.lineage.version." >&2
fi

remote="/data/local/tmp/cacam-dipper-magisk-patch"
...
echo "Patching CaCam OS dipper UVC boot image on device with Magisk $magisk_tag..."
