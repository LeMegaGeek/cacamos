...
zip_path="$repo_root/dist/CaCamOS-dipper-webcam-magisk-$module_version.zip"
remote_path="/data/local/tmp/CaCamOS-dipper-webcam-magisk-$module_version.zip"
adb_bin="${ADB:-adb}"
...
[[ "$device" == "dipper" ]] || fail "expected dipper, got ${device:-unset}"

su_bin="$(detect_su || true)"
[[ -n "$su_bin" ]] || fail "root/Magisk su is not available on the connected dipper"
...
printf '\nInstalled. Reboot with the CaCam OS dipper Magisk boot image, then run:\n'
printf '  %s/cacam-os/lineageos/dipper/tools/verify-webcam.sh\n' "$repo_root"
